...
- **默认只在本地。** 默认只监听回环地址，密钥交给系统存储；没有账号体系，也不经我们的服务器中转（见 [security-privacy.md](./security-privacy.md)）。配置可以经用户自己的第三方云存储（当前是 GitHub Gist）手工同步，但那是用户主动触发、由他本机直连完成的过程，我们不参与也不中转（见 [cloud-sync.md](./cloud-sync.md)）。
...
| 配置云同步 | [cloud-sync.md](./cloud-sync.md) | 同步范围、承载方式（后端）与凭据、推送/拉取语义、不传播的动作 |
