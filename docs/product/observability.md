...
| 已应用修改器 | `request_attempts` | `requestRewriteRuleIds` ∪ `responseRewriteRuleIds`；落库只存 id，展示时按 id 查配置库补名字；事实不随正文采集开关丢失 |
...
**口径说明挂在 info 图标上，不占正文。** 「Provider 路由」下面的「首次尝试即成功 / 第 N 次尝试成功 / N 次尝试全部失败 · 共 N 次尝试」、以及「原始 Usage」下面的「取自最后一次尝试」都是**注解**——它们解释这个数是怎么来的、这块展示的是哪一次尝试，本身不是数据。固定在标题下写一行，每个展开详情的人都得先绕过它，而真正要看的数字与内容被往下推一行；所以这些句子改挂在标题旁的 info 图标上，悬停或键盘聚焦才展开。图标是 16px 的方框、与标题行同高，加进来不会把标题行撞高。

判据是「这句话是不是读者每次都必须读到的」：**失败原因不在此列**——它是结论，不是注解，所以「N 次尝试全部失败」的归纳与失败原因是同一段悬停文案；读者要的诊断信息本来就在下面每一行尝试上（错误码、错误文本、HTTP 状态）。同一个图标也复用在总览页、逻辑模型页那些指标卡上（见 `metric-grid.tsx`），标签右侧那枚图标是同一件事的同一套画法。

## 进行中的请求（内存态）

请求日志回答的是「过去发生了什么」——每条记录都是已经定型的结论。但一条正在跑的请求还没有结论，用户想知道的是「现在走到哪一步了」，而落库这件事天然滞后：`request_logs` 只有在收尾时才写。

因此代理进程里另有一份**内存台账**（`proxy/observability/live-request-store.ts`），只装进行中的请求，收尾后立刻移出，转由请求日志接管。它不落库、不跨进程、不跨重启——它描述的是「此刻」，持久化一个「此刻」没有意义。

| 展示字段 | 来源 | 说明 |
| --- | --- | --- |
| phase | 台账派生 | `routing`（算落点）、`connecting`、`awaiting-upstream`（请求已发出等响应头）、`streaming`（正在交付）、`settled` |
| logicalModelId | 路由结论 | 落点还没算出来就结束时为空 |
| clientProtocol / transport | 请求入口 + 路由结论 | 与列表同名同义 |
| candidates | 路由结论 | 候选顺序即故障转移顺序，用于回答「本来想找谁」 |
| attempts[] | 每次尝试 | 供应商模型、上游协议、URL、状态、`httpStatus`、`upstreamTransport`、请求/上游/下游字节、分片数、分块预览、命中的请求改写规则名、TTFT、输出 Token、错误文本 |
| events[] | 时间轴 | 每条带 `offsetMilliseconds`（相对请求开始），含 `route.resolved` / `attempt.start` / `upstream.head` / `upstream.first-byte` / `route.failover` / `request.*` |

实时尝试行与落库的尝试行**同名同义**，但 `errorCode` 在实时侧恒为空：失败时写进去的是 `errorMessage`，错误码要等执行器把结论文本也补进来才有。字段留在契约里是为了让两边字段名一一对应，界面判断失败只能读错误文本与 `httpStatus`。

实时尝试还多带一格**命中修改器的名字**（`requestRewriteRuleNames`）：落库那条路存的是 id，展示时按 id 查一次配置库补上名字；而实时台账每 150 毫秒就要出一帧，不该为了几个字去查配置库，也不该把查库塞进渲染路径。名字因此由执行器在补字节数的那一刻一起带上——那时它手里正握着刚跑完的规则表与命中结果。时间轴上写的是名字而不是「命中 N 条」：看到「请求被改过」之后的下一句追问就是「是哪几个改的」，而多条命中必须逐条读得出来（历史详情里那排 chip 是同一件事的同一种读法）。
...
- 收尾是**只认第一次**的：请求一旦落定，后续的收尾调用一律忽略。执行器、客户端断开、上游失败三条路径都可能各自走到「结束」，而一个请求只能有一个结局。

保留是有界的（生产环境跑几周也不会涨）：

| 上限 | 值 | 理由 |
| --- | --- | --- |
| 单请求事件数 | 200 | 事件流是给人扫一眼的；长连接流式请求会持续产生事件，无上限则单条请求就能吃掉内存 |
| 已结束请求条数 | 50 | 收尾后还要在界面上留一会儿（原地从「进行中」变成「已结束」，而不是凭空消失），但只留最近的 |
| 已结束请求存活时长 | 60 秒 | 同上；按请求自己的结束时刻算，与进程运行时长无关 |
| 单次尝试保留的分块预览 | 最新 1 条，前 80 个字符 | 预览是用来目视判断「上游到底在回什么」的采样，不是完整报文。只留最新一条，是因为读者看的是**此刻**在流什么——上一块已经被他想过了，堆在界面上只会把光标挤走；而 80 字（约一行的量）足以认出「这是 SSE」「这是 HTML 错误页」。超出界面能显示的部分存下来也没人看得见，只是白占内存 |

界面上的位置：**就在请求记录页那张表里，是表格的一种新行型**。

请求记录页于是只有一张表，它同时说两件事：先说「现在」，再说「过去」。一条进行中的请求在表里就是普通的一行——**与历史行逐字对齐**：同样的列、同样的宽度、同样的横向对齐、同样的「最终落在谁身上 + `+n`」写法，因此它和下面的历史记录可以直接目视接续；展开它，得到的是**一句话加一条时间轴**，而不是历史详情那张静态指标表的复刻。

整行只有一格不同：状态那一格报的是「走到哪一步」（挑上游 / 连上游 / 等响应头 / 等首字节 / 边收边转），而不是「成功还是失败」——一条还在路上的请求唯一有价值的答案是它现在卡在哪，旁边一枚呼吸的圆点让「它还在动」不必读文字就能看出来。其余十格与历史行共用同一套渲染约定，落定时不换面孔。

「等响应头」与「等首字节」是**两个**阶段而非一个，因为它们指向完全不同的排查方向：前者意味着上游还没受理（排队、限流、连接慢），后者意味着上游已经受理但在憋第一个字（模型在思考、提示词很长）。合成一个「等上游」，用户只能知道「慢」。首字节由执行器在上游响应体第一块到达时上报，且**只上报一次**——那一刻之后就转成「边收边转」，后续字节再多也不会改写这一步的耗时。

展开区与历史详情**故意不同构**，因为两者要回答的问题不同：历史详情在复盘（一次请求已经结束，唯一任务是把所有能算出来的数字一次摆齐），实时详情在直播（读者正站在请求外面等结果，只关心「到哪一步了、还快不快、有没有出事」）。但两者仍在**同一套版式里**——顶部都是「标题 + 发丝线 + 一行事实对」的头，正文都是 `DetailSection` 分块，行内字号也照抄列表那一套（标题 13px medium / 节点 12px / 补充 11px / 数字走 mono），间距同样是外层 `px-5 py-4`、分块之间 `mb-4`。差别只在内容挑了什么，不在字形与间距。于是：

- **标题行**是一句现在时——按路由顺序挑上游 / 连上游 / 已发出、等响应头 / 上游应了、等首字节 / 上游在吐、边收边转，落定后换成结局——配一枚随阶段呼吸的圆点；下面一行事实对是逻辑模型、落点、已用时、输出速度。
- **时间轴**把 `route.resolved` / `attempt.start` / `upstream.head` / `route.failover` / `request.*` 讲成一列能顺读的句子：`+20ms` 已发往某家、`+340ms` 上游响应 200、`+1.2s` 改试另一家……刻度是相对起点的偏移量而不是墙上时间。它挂在 `DetailSection` 里，与「Provider 路由」「原始 Usage」是同一个外壳、同一套行内边距。
- 时间轴末端挂着一个**画出来的「此刻」节点**：它不来自事件，跟着时钟往下走，并报出累计收到的字节。一条卡住的请求因此仍然看得出在动，而不是停在上一条事件那里。
- 「收了多少字节」只回答了量，没回答收的是什么：所以末端节点下面另起**一行**分块预览，写上游最新一块开头的原文，超出 200 字的以 `…` 收尾，单行截断显示。**故意不解析**——正常的 SSE 事件、上游回的 HTML 错误页、一串二进制，在出事之前谁也不知道会长什么样，先解成「字段一、字段二」反而把意外情况抹平了。内容是**采样**（最新 1 块），字节数与分片数仍然是真的；预览只在实时详情里出现，不参与任何统计也不落库。只留一行是为了让这行文字始终待在同一个位置：它每 150 毫秒就换一次内容，若是滚动列表，读者的视线就得跟着追。还没收到任何分块时这一行不占位。
- 有意省掉的东西：协议、传输形态、发起时刻都留在折叠行上（那里已经写着，且与历史行同一套列）；方法与路径只出现在时间轴的第一个节点（「收到 POST /v1/responses」），因为它们是这条线从哪里开始的，而不是「此刻在干什么」。请求 ID 在落定后的历史详情里可以复制。展开一条还在跑的请求时，这些字段一个都不帮读者判断「要不要继续等」。

**重试与故障转移是这条时间轴上信息量最大的一段**，也是最容易讲糊的一段（「上游失败了」可以指五种完全不同的情形，用户要做的判断也各不相同）。因此每一种都有一条固定的说法，且都从同一批字段里读出来：

| 事件 | 节点说的是 | 从哪读出来 | 用户据此判断 |
| --- | --- | --- | --- |
| `route.failover` | 谁不行、为什么不行、换谁上 | 失败尝试的 `httpStatus` / `errorMessage`，下一个候选的供应商模型名 | 这家是不是整体不行（失败是否集中在一家） |
| 重试同一家（健康重试） | 同一个落点上的第 n 次尝试 | 尝试序号的递进与落点是否变化 | 是上游抖动，还是路由换了人 |
| 候选耗尽 | 试了几家、最后卡在哪个状态码 | 尝试计数、最后一次上游状态码 | 是不是整条链路都不通 |
| 客户端中断 | 谁断的、断在哪一步 | 中止事件与当时的阶段 | 是客户端超时，还是上游拖太久 |
| 上游返回错误但未转移 | 上游状态码原样透传 | `httpStatus` + 是否还有候选 | 是不是我的密钥/配额问题 |

「换谁上」必须写清目标：只说「失败转移」而不说落到哪一家，读者还得自己往上翻找 `route.resolved`，而这正是他要判断的那个问题。

为什么是同一张表，而不是表格上方另起一块面板：

- 用户的问题是「这条请求怎么了」，而那条请求本来就在表里。另起一块面板等于把同一个对象讲两遍，还要人自己做对应。
- 进行中的请求**必须**在表里——否则「刚发出去的请求不在列表里」会被读成「没发出去」；而它一旦落库又立刻出现在表里，位置会在两块地方之间跳。
- 台账里的请求 id 就是它将来落库后的日志 id，因此展开的行不会在收尾时被折叠掉：行从「进行中」原地变成「已结束」，详情从实时视图原地换成历史视图。
- 排序、分页与筛选仍然只由数据库决定，台账只在第一页、且未套筛选时把「库里还看不见的新请求」补在最前面（`buildRequestLogRows`）。因此列表不会因为内存里多了一条而重排或错位。

进行中的行不显示历史详情里那些只有落库后才有的事实（缓存命中、修改器、最终响应正文），这些列留 `—`，布局一行不缩。没有进行中的请求时，表与标题栏一个都不变。
