...
- 凭据的保存、清除与「能不能用」的校验；
- 远端的绑定与在浏览器中打开；
- 供应商 API Key：随快照一起走，但只做 base64 编码（不是加密）。
...
- **不对密钥做加密**。密钥要能跨机器还原就得有一把「还原它」的东西，而那把东西要么是用户记的口令（云同步就变成了输口令），要么由我们保管（等于我们在托管密钥）；
- 不包含请求日志、用量统计与观测数据；
...
运行设置页在「数据」之后新增分区「云同步」，内含一张卡片，分四行：**存储方式、凭据、远端、上传与拉取**。后三行的标题、行内说明与占位符都来自所选后端自述里的 key，界面不写死任何后端的措辞。
...
| 保存凭据 | 先联网校验再落库；成功后清空输入框 |
| 忘记凭据 | 仅在已保存时出现；从密钥库删除，并清空已记录的账号名 |

说明区交替显示三种状态：未保存（显示后端给的提示，Gist 是「只需 `gist` 权限」）、已保存但没有账号名（提示尚未校验成功）、已保存且有账号名（`已连接为 @login`）。

**保存前先校验**：一个错凭据如果能存进去，用户要等到第一次上传时才看到报错，而那时报错离他刚才的动作已经很远。校验顺带拿到账号名，界面因此能立刻显示「已连接为谁」。

### 第 3 行：远端

| 元素 | 行为 |
| --- | --- |
| 远端输入框 | 内容由后端自己解析；留空并绑定视为解绑 |
...
前置条件不满足时按钮禁用，并用 `title` 说明缺的是什么；其余时候 `title` 说明这一行会覆盖什么（上传覆盖远端快照，拉取覆盖本机的供应商、模型、绑定与密钥）。说明区显示「从未同步」或上次上传/拉取的时间。

这一行下面常驻一句密钥提醒（「API Key 会随快照一起上传，只做 base64 编码。这是编码不是加密——拿到快照的人可以直接还原」）：它不是可关闭的提示，而是告知。用户点上传是在知情的前提下把密钥推出去，所以这句话不能藏在文档里，也不能只在第一次提醒。
...
interface ConfigSnapshot {
  ...
  version: 1
  exportedAt: number
  providers: ProviderBundleProvider[]                    // 永不含 apiKey
  secrets: { providerName: string; value: string }[]     // 各供应商的 API Key，base64
...
**绑定用名字而不是 id 指代供应商与模型。** id 是本机的（`prov_…`、`model_…`），换一台机器后必然对不上；名字是用户自己起的，也是他跨机器能认出来的东西。代价是「改名」在同步语义上等于「换一个对象」。`secrets` 也用名字指向 `providers` 里的同名条目，理由同上。

**密钥单独放在 `secrets` 里、`providers` 保持腋敏。** 不把 base64 塞进 `providers[].apiKey`：那个字段与供应商包共用，在供应商包里它是明文，同一个字段名在两种文档里表示两种东西是迟早要出错的。`secrets` 缺席时（旧快照，或用户手改过的）全部供应商沿用本机已有的密钥，旧快照因此照样能拉。解不出来的编码也当作「这份快照没带这把密钥」：宁可退化成沿用本机，也不要把一段乱码写进密钥库——那要等到下一次请求才以一个看不懂的错误爆掉。

**`secrets` 是编码，不是加密。** `value` 是密钥的 base64，一条 `base64 -d` 就能还原。它解决的问题是「别让 `sk-…` 直接躺在远端文件里被肉眼扫到、被密钥扫描器判成泄露」，不是「防止别人读取」。真正的边界是那个容器本身的可见性（Gist 是 `public: false`），而私密 Gist 也不是加密存储。
...
1. 读本机配置生成快照（含各供应商的 API Key，base64 编码）；
2. 已绑定远端 → 覆盖那个文件；未绑定 → 由**后端新建一个**并把句柄回填到设置（Gist 是新建一个私密 Gist）；
3. 记录上传时间；首次新建时把「上次拉取时间」清零。

未绑定时自动新建，是为了让「第一次同步」只需要点一次按钮，而不是先让用户自己去后端的网页上建一个容器、复制句柄、再粘回来。这个能力由后端自述里的 `createsTarget` 声明：能新建的后端在上传时兼管建容器，不能新建的（比如固定目录的 WebDAV）就要求用户先自己建好。Gist 一律 `public: false`——配置里虽然不含明文密钥，但供应商地址、模型名与逻辑模型名仍是用户的部署信息。

### 拉取

1. 读远端的 `osw-config.json`，文件不存在、读不出来或不是 JSON 时直接报错，不写任何东西；
2. 校验成快照，不通过则报「这不是一份配置快照」并附前几条字段级原因；
3. 应用：
   - **供应商**按名称覆盖，沿用供应商导入的规则（包内未提到的模型软删除）；
...
| `CLOUD_SYNC_REMOTE_FILE_INVALID` | 远端那个文件不是合法 JSON | 文件被改坏了 |
...
- **快照里带供应商 API Key，但只做 base64 编码。** 不带密钥的话「换台机器点一次拉取就能接着用」只成立一半：渠道都在，每个还得重新去官网签一次 Key。代价是密钥落到了托管方手里，我们接受的边界是：只给编码不给明文、界面上常驻一句说明、并且这件事写在本文里；
- **不做加密是刻意的。** 密钥要能跨机器还原，就得有一把「还原它」的东西，而那把东西要么是用户记的口令、要么是我们保管的密钥——前者把云同步变成了输口令，后者等于我们在托管密钥。base64 只是不让密钥以肉眼可辨的形式出现，**不要把它当成保护**；
- Gist 新建时一律私密；但**私密 Gist 不是加密存储**——GitHub 及其运维方在技术上能读到内容，因此快照承载的是「部署信息 + 密钥」这一级别的数据，不要把它当成凭据保险箱；
...
- 密钥编解码：`packages/core/source/management/cloud-sync/secret-codec.ts`；
