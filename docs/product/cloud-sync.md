...
前置条件不满足时按钮禁用，并用 `title` 说明缺的是什么；其余时候 `title` 说明这一行会覆盖什么（上传覆盖远端快照，拉取覆盖本机的供应商、模型与绑定）。说明区显示「从未同步」或上次上传/拉取的时间。

**这一行的下方常驻一行告知（`settings.cloudSync.encodingNotice`）**：快照整份只做 base64 编码、不是加密，拿到文件的人能还原全部内容包括 API Key。这句不是可关闭的提示而是告知——把用户的密钥推到一个他自己建的容器里，前提是他知道这件事正在发生。
...
- 界面与文档都告知这一点：云同步卡片下方常驻一行说明，本文件与 [security-privacy.md](./security-privacy.md) 同步写明；
